...
const CarOrderPage = () => {
  ...
  const [factory, setFactory] = useState(car_order?.factory || '');
  ...
  const formCarOrderHandler = async () => {
    if (id && factory) {
      // Передаем factory в качестве параметра для запроса
      await dispatch(formCarOrder({ id, factory }));
      setIsForm(true);
      navigate('/details');
    } else {
      console.error('ID или factory не найден');
    }
  };
